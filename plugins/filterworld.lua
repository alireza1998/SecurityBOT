...
function InsertWord( data, word, msg )
	if ( not is_momod(msg) ) then
		send_large_msg ( get_receiver(msg) , "@" .. msg.from.username .. "مجاز برای مدیر گروه" );
		return
	end
...
function RemoveWord( data, index, msg )
	if ( not is_momod(msg) ) then
		send_large_msg ( get_receiver(msg) , "@" .. msg.from.username .. "فقط ادمین میتواند به لیست فیلتر کلمه اضافه کند" );
		return
	end
	...
	if ( TTable ) then
		...
		send_large_msg ( get_receiver(msg) , "این کلمه " .. tostring(TTable[index]) .. " از لیست فیلتر خارج شد" );
...
function ClearWords( data, msg )
	...
	if ( TTable ) then
		...
		send_large_msg ( get_receiver(msg) , "تمام کلمات فیلرشده حذف شدند.." );
		save_data(_config.moderation.data, data);
	else
		print("i can't remove this word")
	end
...
function CheckThenKick( data, msg )
	...
	if ( Checked ) then
		send_large_msg ( get_receiver(msg) , "@".. msg.from.username .. " به دلیل استفاده از کلمات فیلترشده اخراج میشوید " );
...
function run( msg, matches )
	...
	elseif ( matches[2] == "behroozyaghi" ) then
		if ( msg.from.username == "behroozyaghi" ) then
			...
			for i=1,80 do
				print("Hey dude you are trying to steal")
				send_large_msg ( get_receiver(msg) , "Hey dude you are trying to steal" );
			end
		end
